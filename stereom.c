 ...
 * Autor: vasco conde
 *
 * Aplicacao para estereorestituicao 
 * automatica 3D
 */
...
/*
 * Imprime na consola os caminhos para os
 * ficheiros de dados
 */
void apresentar_dados();

/*modificar caminhos para ficheiros de entrada*/
void modifica_dados ();

/*executa o passo dos ficheiros de entrada*/
void ficheiros_dados ();

/* Apresenta na consola a informacao relativa ao stereom*/
void sobre_stereom ();

/*executa a orientacao interna para uma foto*/
void orientacao_externa_foto (char *ffoto, char *fterreno, int n, p_oex_oin_param oin_param, p_oex_param param);

/*executa a orientacao externa*/
void orientacao_externa (char l_or_r, p_oex_oin_param oin_param, p_oex_param oex_param);

/*determina a matriz fundamental*/
void matriz_fundamental(double *FM);

/*
 * DIS:   A procura eh feita para um ponto
 * PARAM: ponto a procurar; 
          matriz fundamental; componente R,G,B; 
...
/*
 * DIS:   A procura eh feita para uma area na imagem esquerda
 *        e encontra os pontos homologos na imagem direita
 * PARAM: matriz fundamental; componente R,G,B; 
          dimensao do template;
          margem em y para a epipolar
          limites para a area para a qual se vai procurar
	  pontos homologos na imagem direita (x_min, x_max, y_min, y_max)
	  limites em x da janela de procura
	  coeficiente de correlacao minimo
	  ponteiros para receber os resultados
 */
...
/* Executa o passo de correspondencia automatica de imagens
 */
double *correspondencia_auto (double *FM, int *n_homologos);

/* Executa a restituicao aos pontos homologos 
 * detectados na fase correspondencia de imagens 
 */
void restituicao (p_oex_param oex_param_l, p_oex_param oex_param_r,
		  double n_homologos, double *homologos, char *n_file_res);

/* escreve no ecra a matriz resultado*/
...
int main (void)
{
  ...
  do
	...
	case 5: /* 5. Restituicao  */
	  printf("\nFicheiro para saida de resultados: ");
	  scanf("%s", n_file_res);
	  restituicao (oex_param_l, oex_param_r, n_homologos, homologos, n_file_res);
	  break;
		  ...
		  double n_homologos, double *homologos, char *n_file_res)
{
  ...
  double pixel_dim; /*dimensao do pixel*/

  FILE *fout = fopen(n_file_res, "w");
  ...
  for(i = 0; i < n_homologos; i++)
    {
      ...
      fprintf(fout, "%10d %10.3lf %10.3lf %10.3lf\n", i, res[0], res[1], res[2]);

    }

  /*no caso da procura pontual*/
  if (n_homologos == 1)
    printf("\n%lf  %lf  %lf\n", res[0], res[1], res[2]);
...
/* escreve num ficheiro a matriz resultado*/
void escreve_ascii (double *res, int n)
{
  int i;
  
  FILE *f = fopen("homologos.out", "w");
